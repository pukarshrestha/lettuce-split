...
(function () {
  ...
  function addItem(name, unitPrice, quantity) {
    ...
    const item = {
      ...
      consumers: [], // default: no one selected
    };
  ...
  function toggleAllConsumers(itemId) {
    ...
    if (!item) return;
    const allChecked = state.people.length > 0 && item.consumers.length === state.people.length;
    item.consumers = allChecked ? [] : [...state.people.map(p => p.id)];
    render();
  }
  ...
  function renderItemsTable() {
    ...
    state.items.forEach(item => {
      ...
      const allConsumersChecked = hasPeople && item.consumers.length === state.people.length;
      const toggleAllTitle = allConsumersChecked ? 'Unselect all' : 'Select all';
      const toggleAllIcon = allConsumersChecked
          ...
          <td>
            <div class="row-actions">
              ${hasPeople ? `<button
                ...
                data-toggle-all-consumers="${item.id}"
                aria-label="${toggleAllTitle} consumers for ${escapeHTML(item.name)}"
  ...
  $itemsTableWrap.addEventListener('click', function (e) {
    ...
    if (removeBtn) {
      removeItem(parseInt(removeBtn.dataset.removeItem, 10));
      return;
    }
    const dupBtn = e.target.closest('[data-duplicate-item]');
    if (dupBtn) {
      duplicateItem(parseInt(dupBtn.dataset.duplicateItem, 10));
      return;
    }
    const toggleBtn = e.target.closest('[data-toggle-all-consumers]');
    if (toggleBtn) {
      toggleAllConsumers(parseInt(toggleBtn.dataset.toggleAllConsumers, 10));
      return;
    }
  });
